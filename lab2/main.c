...
int main(int argc, const char** argv) {
    ...
    for (int i = 0; argv[1][i] > 0; ++i) {
        if (argv[1][i] >= '0' && argv[1][i] <= '9') {       //str to int
    ...
    scanf("%lf", &R);
    Total_Aria_Size = R * 2;
    pthread_t* th = malloc(sizeof (pthread_t) * CountThreads);  //массив потоков
    ThreadToken* token = malloc(sizeof(ThreadToken) * CountThreads);  //массив токенов
